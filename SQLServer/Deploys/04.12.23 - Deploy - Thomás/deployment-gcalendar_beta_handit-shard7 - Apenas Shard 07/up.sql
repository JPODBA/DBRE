...
UPDATE WebHook SET Active = 0 WHERE id IN (SELECT WebhookId FROM Ploomes_Cliente_Integration_Webhook WHERE AccountIntegrationId IN (SELECT ID FROM Ploomes_Cliente_Integration WHERE IntegrationId = 12 and AccountId = 1001460))

UPDATE Ploomes_Cliente_Integration SET StatusId = 2, Authorized = 0 WHERE IntegrationId = 12 and AccountId = 1001460 AND StatusId = 1 AND Authorized = 1
GO
...
BEGIN
     ...
     SELECT @rowcount = count(1) FROM Campo_Valor_Task (NOLOCK) WHERE FieldId = @OldFieldId
     WHILE @rowcount > 0 
     BEGIN

        UPDATE TOP(100) Campo_Valor_Task 
        SET FieldId = @NewFieldId 
        WHERE FieldId = @OldFieldId

        Select @rowcount = @rowcount - 100
...
BEGIN
 ...
 BEGIN
	...
	BEGIN
	 ...
	 EXEC Ploomes_CRM.dbo.Int_Update_Campo_Task_Valor @NewFieldId = @NewFieldId, @CustomFieldId = 38, @AcountId = @AccountId;
	END;
	...
	BEGIN
	 ...
	 EXEC Ploomes_CRM.dbo.Int_Update_Campo_Task_Valor @NewFieldId = @NewFieldId, @CustomFieldId = 39, @AcountId = @AccountId;
	END;
	...
	BEGIN
	 ...
	 EXEC Ploomes_CRM.dbo.Int_Update_Campo_Task_Valor @NewFieldId = @NewFieldId, @CustomFieldId = 78, @AcountId = @AccountId;
	END;
 END;
END;  
GO
