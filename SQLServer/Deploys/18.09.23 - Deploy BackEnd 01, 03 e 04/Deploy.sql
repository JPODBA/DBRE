...
IF EXISTS(
	...
	FROM inserted i 
		LEFT JOIN deleted d 
			ON i.Id = d.Id 
	WHERE
		i.TriggerId = 17 -- periodic automation
)
    BEGIN
        UPDATE Automation 
        SET NextRunTime = IIF(A.LastRunTime IS NULL AND A.TriggerRepeatStartDateTime IS NOT NULL,
                A.TriggerRepeatStartDateTime, 
                DATEADD(
                    MINUTE,
                    ISNULL(
                        DATEPART(MINUTE, A.TriggerRepeatStartDateTime), 0
                    ), 
                    DATEADD(
                        HOUR, 
                        ISNULL(DATEPART(HOUR, A.TriggerRepeatStartDateTime), 0),
                        CASE 
                            WHEN IU.Datepart = 'day' 
                            THEN DATEADD(
                                    DAY, 
                                    IU.DatepartLength * A.TriggerRepeatIntervalLength, 
                                    CAST(CAST(A.LastRunTime as DATE) as DATETIME)
                                )
                            WHEN IU.Datepart = 'month' 
                            THEN DATEADD(
                                MONTH, 
                                IU.DatepartLength * A.TriggerRepeatIntervalLength, 
                                CAST(CAST(A.LastRunTime as DATE) as DATETIME)
        ...
        FROM Automation A 
            INNER JOIN inserted i 
                ON A.Id = i.Id
            LEFT JOIN IntervalUnit IU 
                ON A.TriggerRepeatIntervalUnitId = IU.Id
        WHERE 
            A.TriggerId = 17 
            AND A.Suspended = 0
    END
...
ALTER TRIGGER [dbo].[Tg_Notificacao_Insert]
	...
	AFTER INSERT
AS BEGIN
	DECLARE @Id INT, @UserId INT, @AccountId INT, @MobileNotificationTokenIsNull BIT

	SELECT @Id = Id, @UserId = ID_Usuario FROM inserted

	UPDATE Usuario 
	SET ContagemNotificacoes = ContagemNotificacoes + 1
	WHERE ID = @UserId

	SELECT @MobileNotificationTokenIsNull = IIF(Mobile_PushNotificationToken IS NULL, 1, 0), @AccountId = ID_ClientePloomes
	FROM Usuario WITH (NOLOCK)
	WHERE ID = @UserId

	UPDATE Notificacao 
	SET MobileNotificationPushed = @MobileNotificationTokenIsNull,
		AccountId = @AccountId
	WHERE ID = @Id
END
GO
...
AS BEGIN
	...
	SELECT @MotivoSuspenso = Suspenso
		FROM Oportunidade_MotivoPerda WITH (NOLOCK) WHERE ID = @ID_MotivoPerdaAntigo
	...
	IF @ID_StatusNovo <> @ID_StatusAntigo OR @ID_StatusAntigo = 2 AND @ID_MotivoPerdaAntigo <> @ID_MotivoPerdaNovo AND @MotivoSuspenso = 'False' BEGIN
		...
		SELECT @OrdemAntiga = Ordem FROM Oportunidade_Status WITH (NOLOCK) WHERE ID = @ID_StatusAntigo
		SELECT @OrdemNova = Ordem FROM Oportunidade_Status WITH (NOLOCK) WHERE ID = @ID_StatusNovo
		SELECT @ID_Venda = ID FROM Venda WITH (NOLOCK) WHERE ID_Oportunidade = @ID_Oportunidade AND Suspenso = 'False'
	...
	SELECT @Prospeccao = CS.Prospeccao, @ID_ClientePloomes = Cli.ID_ClientePloomes,
		...
		FROM Cliente Cli WITH (NOLOCK) INNER JOIN Cliente_Status CS WITH (NOLOCK) ON Cli.ID_Status = CS.ID
		WHERE Cli.ID = @ID_Cliente

	SELECT @ID_StatusInativo = St.ID
		FROM Cliente_Status St WITH (NOLOCK)
...
AS BEGIN
	...
	IF @ID_StatusNovo <> @ID_StatusAntigo BEGIN
		DECLARE @OrdemAntiga INT, @OrdemNova INT
		SELECT @OrdemAntiga = Ordem FROM Venda_Status WITH (NOLOCK) WHERE ID = @ID_StatusAntigo
		SELECT @OrdemNova = Ordem FROM Venda_Status WITH (NOLOCK) WHERE ID = @ID_StatusNovo
		...
		SELECT @Email_Cliente = Cli.Email, @Nome_ClientePloomes = PC.Nome, @Codigo = V.Codigo,
			...
			FROM Venda V WITH (NOLOCK) INNER JOIN Cliente Cli WITH (NOLOCK) ON V.ID_Cliente = Cli.ID
				INNER JOIN Ploomes_Cliente PC WITH (NOLOCK) ON Cli.ID_ClientePloomes = PC.ID
	...
	SELECT DISTINCT T.*
		FROM (
			...
			SELECT 6 as ID_Entidade, Usu.ID as ID_Item, Usu.Nome as BuscaTexto, NULL as BuscaInteiro, NULL as ID_Cliente, NULL as NomeCliente, NULL as ID_Oportunidade, NULL as ID_CotacaoRevisao,
					NULL as ID_Venda, NULL as ID_Lead, Usu.ID as ID_Usuario, UsuPerm.ID_Usuario as ID_UsuarioView, 110 as Ordem
				FROM Vw_Usuario3 UsuPerm INNER JOIN Usuario U ON UsuPerm.ID_Usuario = U.ID
					INNER JOIN Usuario_Perfil P ON U.ID_Perfil = P.ID AND P.Timeline_Empresa = 'True'
					INNER JOIN Usuario Usu ON UsuPerm.ID = Usu.ID
	) as T
GO
...
--UPDATE CampoFixo2 SET CreateDate = '2016-12-29 00:00:00.000';