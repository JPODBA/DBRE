...
Begin
  ...
  DROP TABLE if exists #OriginCollaboratorUsers
	...
	INTO #OriginContacts
	FROM [Shard05].[Ploomes_CRM].[dbo].[Cliente] C (NOLOCK) 
	INNER JOIN [Shard05].[Ploomes_CRM].[dbo].[Ploomes_Cliente] PC (NOLOCK) ON C.ID_ClientePloomes = PC.ID
	...
	FROM [Shard05].[Ploomes_CRM].[dbo].[Cliente_Colaborador_Usuario] CCU (NOLOCK) INNER JOIN #OriginContacts C ON CCU.ID_Cliente = C.ID 
