...
Begin
	...
	INTO #OriginUsers
	FROM [Shard11].[Ploomes_CRM].[dbo].[Usuario] U (NOLOCK) INNER JOIN [Shard11].[Ploomes_CRM].[dbo].[Ploomes_Cliente] PC (NOLOCK) ON U.ID_ClientePloomes = PC.ID
	...
	FROM [Shard11].[Ploomes_CRM].[dbo].[Usuario_Responsavel] UR (NOLOCK) INNER JOIN #OriginUsers U ON UR.ID_Usuario = U.ID
