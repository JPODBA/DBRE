...
Begin
	...
	INTO #OriginTeams
	FROM [Shard01].[Ploomes_CRM].[dbo].[Equipe] EQ (NOLOCK) INNER JOIN [Shard01].[Ploomes_CRM].[dbo].[Ploomes_Cliente] PC (NOLOCK) ON EQ.ID_ClientePloomes = PC.ID
	...
	INTO #OriginTeamUser
	FROM [Shard01].[Ploomes_CRM].[dbo].[Equipe_Usuario] EU (NOLOCK) INNER JOIN #OriginTeams OT ON EU.ID_Equipe = OT.ID
