...
Begin
	...
	INTO #OriginProfiles
	FROM [Shard01].[Ploomes_CRM].[dbo].[Usuario_Perfil] UP (NOLOCK) INNER JOIN [Shard01].[Ploomes_CRM].[dbo].[Ploomes_Cliente] PC (NOLOCK) ON UP.ID_ClientePloomes = PC.ID
	...
	FROM [Shard01].[Ploomes_CRM].[dbo].[Usuario_Perfil_AllowedChatUser] UPA (NOLOCK) INNER JOIN #OriginProfiles UP ON UPA.ProfileId = UP.ID
