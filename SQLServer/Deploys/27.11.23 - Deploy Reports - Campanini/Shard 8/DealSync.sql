...
Begin
	...
	INTO #OriginDeals
	FROM [Shard08].[Ploomes_CRM].[dbo].[Oportunidade] O (NOLOCK) INNER JOIN [Shard08].[Ploomes_CRM].[dbo].[Ploomes_Cliente] PC (NOLOCK) ON O.ID_ClientePloomes = PC.ID
	...
	FROM [Shard08].[Ploomes_CRM].[dbo].[Oportunidade_Colaborador_Usuario] OCU (NOLOCK) INNER JOIN #OriginDeals OD ON OCU.ID_Oportunidade = OD.ID
