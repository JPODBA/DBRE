--ALTER TABLE BA_DBA..DBA_MONITOR_INDEX_NOUSE
--ALTER COLUMN indice varchar(500) Collate Latin1_General_CI_AI;

drop table if exists #DROP
Select 
	COUNT(indice) as Count1, 
	indice as indice, tabela as tabela,
	'Drop index if exists '+tabela+'.'+indice+';' as comando
Into #DROP
From BA_DBA..DBA_MONITOR_INDEX_NOUSE
group by indice, tabela
Having COUNT(indice) > 15
order by COUNT(indice) desc
go
ALTER TABLE #DROP
ALTER COLUMN indice varchar(500) Collate Latin1_General_100_CI_AI;
select Count(1) from #DROP
go 
Use Ploomes_CRM
...
FROM sys.indexes I  
JOIN #DROP dba ON dba.indice = I.name
