use BA_DBA
go
CREATE or ALTER  PROC PR_DBA_EXPURGO_USUARIO_FILTRO_TAREFA_CLASSE
	@Debug bit = 1, 
	@AccountId INT,
	@timeout  Time = '02:45',
	@Top INT
/************************************************************************
 ...
 Data de att:      
 Funcionalidade: Faz o Expurgo de dados da tabela USUARIO_FILTRO_TAREFA_CLASSE do Ploomes_CRM. 
*************************************************************************/
AS
BEGIN
	SET NOCOUNT ON

	IF EXISTS (SELECT 1 FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE AcountID = @AccountId AND SPROC = 'PR_DBA_EXPURGO_USUARIO_FILTRO_TAREFA_CLASSE' AND CountFim = 0)
	BEGIN
		PRINT 'PR_DBA_EXPURGO_USUARIO_FILTRO_TAREFA_CLASSE já zerou esse accountID. Segue o próximo!!'
		RETURN
	END
    ...
    FROM Ploomes_CRM.dbo.[USUARIO_FILTRO_TAREFA_CLASSE] T
    JOIN Ploomes_CRM.dbo.[Usuario] U ON T.[ID_Usuario] = U.[ID]
	...
	Select @Data_Inicio, null, 'PR_DBA_EXPURGO_USUARIO_FILTRO_TAREFA_CLASSE', @CountLinhas_Inicio, null, @AccountId			
	
	SELECT @ID_LogManutencao = id FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE DataInicio = @Data_Inicio and SPROC = 'PR_DBA_EXPURGO_USUARIO_FILTRO_TAREFA_CLASSE'
	--- lOG Delete Historico  ------------------------------------------------------------------------------
