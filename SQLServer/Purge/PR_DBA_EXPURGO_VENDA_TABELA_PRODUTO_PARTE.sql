use BA_DBA
go
CREATE or ALTER  PROC PR_DBA_EXPURGO_VENDA_TABELA_PRODUTO_PARTE
	@Debug bit = 1, 
	@AccountId INT,
	@timeout  Time = '02:45',
	@Top INT
/************************************************************************
 ...
 Data de att:      
 Funcionalidade: Faz o Expurgo de dados da tabela VENDA_TABELA_PRODUTO_PARTE do Ploomes_CRM. 
*************************************************************************/
AS
BEGIN
	SET NOCOUNT ON

	IF EXISTS (SELECT 1 FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE AcountID = @AccountId AND SPROC = 'PR_DBA_EXPURGO_VENDA_TABELA_PRODUTO_PARTE' AND CountFim = 0)
	BEGIN
		PRINT 'PR_DBA_EXPURGO_VENDA_TABELA_PRODUTO_PARTE já zerou esse accountID. Segue o próximo!!'
		RETURN
	END
    ...
    FROM Ploomes_CRM.dbo.[VENDA_TABELA_PRODUTO_PARTE]   T 
	JOIN Ploomes_CRM.dbo.[Venda_Tabela_Produto]       VTP ON T.[ID_VendaTabelaProduto] = VTP.[ID] 
	JOIN Ploomes_CRM.dbo.[Venda]				        V ON VTP.[ID_Venda] = V.[ID]
	...
	Select @Data_Inicio, null, 'PR_DBA_EXPURGO_VENDA_TABELA_PRODUTO_PARTE', @CountLinhas_Inicio, null, @AccountId			
	
	SELECT @ID_LogManutencao = id FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE DataInicio = @Data_Inicio and SPROC = 'PR_DBA_EXPURGO_VENDA_TABELA_PRODUTO_PARTE'
	--- lOG Delete Historico  ------------------------------------------------------------------------------
