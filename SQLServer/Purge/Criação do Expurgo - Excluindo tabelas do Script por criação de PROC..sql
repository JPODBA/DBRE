PRINT 'Running accountId 1 and table [Timeline]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempTimeline
                            ...
                            CREATE TABLE #TempTimelineDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempTimeline))
                            BEGIN
                                ...
                                DELETE T
                                FROM [Timeline] T INNER JOIN #TempTimelineDel TempTimelineDel ON [T].ID = TempTimelineDel.Id
                                WHERE [T].ID >= @MinId AND [T].ID <= @MaxId
                                        
                                DELETE #TempTimeline
                                FROM #TempTimeline TempTimeline INNER JOIN #TempTimelineDel TempTimelineDel ON TempTimeline.ID = TempTimelineDel.Id
                            END
                            DROP TABLE #TempTimeline
                            DROP TABLE #TempTimelineDel
                            GO
PRINT 'Running accountId 1 and table [RelatedPerson_ExternalOpening]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempRelatedPerson_ExternalOpening

                            CREATE TABLE #TempRelatedPerson_ExternalOpening
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempRelatedPerson_ExternalOpening (Id)
                            SELECT DISTINCT [RE].Id
                            FROM [RelatedPerson_ExternalOpening] RE 
							INNER JOIN [RelatedPerson] R ON RE.RelatedPersonId = R.Id 
							LEFT JOIN (SELECT ID AS ItemId, ID_ClientePloomes, 7 as EntityId 
							FROM [Cotacao_Revisao] UNION ALL SELECT ID AS ItemId, ID_ClientePloomes, 4 as EntityId 
							FROM [Venda] UNION ALL SELECT ID AS ItemId, AccountId AS ID_ClientePloomes, 66 as EntityId 
							FROM [Document] UNION ALL SELECT ID AS ItemId, ID_ClientePloomes, 1 as EntityId FROM [Cliente]) T 
							ON (T.EntityId = 7 AND T.ItemId = R.QuoteId) OR (T.EntityId = 4 AND T.ItemId = R.OrderId) OR (T.EntityId = 66 AND  T.ItemId = R.DocumentId) OR (T.EntityId = 1 AND  T.ItemId = R.ContactId)
                            WHERE [T].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempRelatedPerson_ExternalOpeningDel
                            CREATE TABLE #TempRelatedPerson_ExternalOpeningDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempRelatedPerson_ExternalOpening))
                            BEGIN
                                TRUNCATE TABLE #TempRelatedPerson_ExternalOpeningDel

                                INSERT #TempRelatedPerson_ExternalOpeningDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempRelatedPerson_ExternalOpening
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempRelatedPerson_ExternalOpeningDel
                                DELETE RE
                                FROM [RelatedPerson_ExternalOpening] RE INNER JOIN #TempRelatedPerson_ExternalOpeningDel TempRelatedPerson_ExternalOpeningDel ON [RE].ID = TempRelatedPerson_ExternalOpeningDel.Id
                                WHERE [RE].ID >= @MinId AND [RE].ID <= @MaxId
                                        
                                DELETE #TempRelatedPerson_ExternalOpening
                                FROM #TempRelatedPerson_ExternalOpening TempRelatedPerson_ExternalOpening INNER JOIN #TempRelatedPerson_ExternalOpeningDel TempRelatedPerson_ExternalOpeningDel ON TempRelatedPerson_ExternalOpening.ID = TempRelatedPerson_ExternalOpeningDel.Id
                            END
                            DROP TABLE #TempRelatedPerson_ExternalOpening
                            DROP TABLE #TempRelatedPerson_ExternalOpeningDel
                            GO
PRINT 'Running accountId 1 and table [RelatedPerson]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempRelatedPerson

                            CREATE TABLE #TempRelatedPerson
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempRelatedPerson (Id)
                            SELECT DISTINCT [R].Id
                            FROM [RelatedPerson] R LEFT JOIN (SELECT ID AS ItemId, ID_ClientePloomes, 7 as EntityId FROM [Cotacao_Revisao] UNION ALL SELECT ID AS ItemId, ID_ClientePloomes, 4 as EntityId FROM [Venda] UNION ALL SELECT ID AS ItemId, AccountId AS ID_ClientePloomes, 66 as EntityId FROM [Document] UNION ALL SELECT ID AS ItemId, ID_ClientePloomes, 1 as EntityId FROM [Cliente]) T ON (T.EntityId = 7 AND T.ItemId = R.QuoteId) OR (T.EntityId = 4 AND T.ItemId = R.OrderId) OR (T.EntityId = 66 AND  T.ItemId = R.DocumentId) OR (T.EntityId = 1 AND  T.ItemId = R.ContactId)
                            WHERE [T].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempRelatedPersonDel
                            CREATE TABLE #TempRelatedPersonDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempRelatedPerson))
                            BEGIN
                                TRUNCATE TABLE #TempRelatedPersonDel

                                INSERT #TempRelatedPersonDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempRelatedPerson
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempRelatedPersonDel
                                DELETE R
                                FROM [RelatedPerson] R INNER JOIN #TempRelatedPersonDel TempRelatedPersonDel ON [R].ID = TempRelatedPersonDel.Id
                                WHERE [R].ID >= @MinId AND [R].ID <= @MaxId
                                        
                                DELETE #TempRelatedPerson
                                FROM #TempRelatedPerson TempRelatedPerson INNER JOIN #TempRelatedPersonDel TempRelatedPersonDel ON TempRelatedPerson.ID = TempRelatedPersonDel.Id
                            END
                            DROP TABLE #TempRelatedPerson
                            DROP TABLE #TempRelatedPersonDel
                            GO
PRINT 'Running accountId 1 and table [Notificacao]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempNotificacao

                            CREATE TABLE #TempNotificacao
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempNotificacao (Id)
                            SELECT  [N].Id
                            FROM [Notificacao] N
                            WHERE [N].[AccountId] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempNotificacaoDel
                            CREATE TABLE #TempNotificacaoDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempNotificacao))
                            BEGIN
                                TRUNCATE TABLE #TempNotificacaoDel

                                INSERT #TempNotificacaoDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempNotificacao
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempNotificacaoDel
                                DELETE N
                                FROM [Notificacao] N INNER JOIN #TempNotificacaoDel TempNotificacaoDel ON [N].ID = TempNotificacaoDel.Id
                                WHERE [N].ID >= @MinId AND [N].ID <= @MaxId
                                        
                                DELETE #TempNotificacao
                                FROM #TempNotificacao TempNotificacao INNER JOIN #TempNotificacaoDel TempNotificacaoDel ON TempNotificacao.ID = TempNotificacaoDel.Id
                            END
                            DROP TABLE #TempNotificacao
                            DROP TABLE #TempNotificacaoDel
                            GO
PRINT 'Running accountId 1 and table [Integracao_TotalVoice_Usuario]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempIntegracao_TotalVoice_Usuario

                            CREATE TABLE #TempIntegracao_TotalVoice_Usuario
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempIntegracao_TotalVoice_Usuario (Id)
                            SELECT  [ITU].Id
                            FROM [Integracao_TotalVoice_Usuario] ITU INNER JOIN [Integracao_TotalVoice] IT ON ITU.[ID_Integracao] = IT.[ID]
                            WHERE [IT].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempIntegracao_TotalVoice_UsuarioDel
                            CREATE TABLE #TempIntegracao_TotalVoice_UsuarioDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempIntegracao_TotalVoice_Usuario))
                            BEGIN
                                TRUNCATE TABLE #TempIntegracao_TotalVoice_UsuarioDel

                                INSERT #TempIntegracao_TotalVoice_UsuarioDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempIntegracao_TotalVoice_Usuario
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempIntegracao_TotalVoice_UsuarioDel
                                DELETE ITU
                                FROM [Integracao_TotalVoice_Usuario] ITU INNER JOIN #TempIntegracao_TotalVoice_UsuarioDel TempIntegracao_TotalVoice_UsuarioDel ON [ITU].ID = TempIntegracao_TotalVoice_UsuarioDel.Id
                                WHERE [ITU].ID >= @MinId AND [ITU].ID <= @MaxId
                                        
                                DELETE #TempIntegracao_TotalVoice_Usuario
                                FROM #TempIntegracao_TotalVoice_Usuario TempIntegracao_TotalVoice_Usuario INNER JOIN #TempIntegracao_TotalVoice_UsuarioDel TempIntegracao_TotalVoice_UsuarioDel ON TempIntegracao_TotalVoice_Usuario.ID = TempIntegracao_TotalVoice_UsuarioDel.Id
                            END
                            DROP TABLE #TempIntegracao_TotalVoice_Usuario
                            DROP TABLE #TempIntegracao_TotalVoice_UsuarioDel
                            GO
PRINT 'Running accountId 1 and table [Integracao_TotalVoice]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempIntegracao_TotalVoice

                            CREATE TABLE #TempIntegracao_TotalVoice
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempIntegracao_TotalVoice (Id)
                            SELECT  [IT].Id
                            FROM [Integracao_TotalVoice] IT
                            WHERE [IT].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempIntegracao_TotalVoiceDel
                            CREATE TABLE #TempIntegracao_TotalVoiceDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempIntegracao_TotalVoice))
                            BEGIN
                                TRUNCATE TABLE #TempIntegracao_TotalVoiceDel

                                INSERT #TempIntegracao_TotalVoiceDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempIntegracao_TotalVoice
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempIntegracao_TotalVoiceDel
                                DELETE IT
                                FROM [Integracao_TotalVoice] IT INNER JOIN #TempIntegracao_TotalVoiceDel TempIntegracao_TotalVoiceDel ON [IT].ID = TempIntegracao_TotalVoiceDel.Id
                                WHERE [IT].ID >= @MinId AND [IT].ID <= @MaxId
                                        
                                DELETE #TempIntegracao_TotalVoice
                                FROM #TempIntegracao_TotalVoice TempIntegracao_TotalVoice INNER JOIN #TempIntegracao_TotalVoiceDel TempIntegracao_TotalVoiceDel ON TempIntegracao_TotalVoice.ID = TempIntegracao_TotalVoiceDel.Id
                            END
                            DROP TABLE #TempIntegracao_TotalVoice
                            DROP TABLE #TempIntegracao_TotalVoiceDel
                            GO
PRINT 'Running accountId 1 and table [WebHook]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempWebHook

                            CREATE TABLE #TempWebHook
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempWebHook (Id)
                            SELECT  [WebHook].Id
                            FROM [WebHook] WebHook
                            WHERE [WebHook].[AccountId] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempWebHookDel
                            CREATE TABLE #TempWebHookDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempWebHook))
                            BEGIN
                                TRUNCATE TABLE #TempWebHookDel

                                INSERT #TempWebHookDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempWebHook
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempWebHookDel
                                DELETE WebHook
                                FROM [WebHook] WebHook INNER JOIN #TempWebHookDel TempWebHookDel ON [WebHook].ID = TempWebHookDel.Id
                                WHERE [WebHook].ID >= @MinId AND [WebHook].ID <= @MaxId
                                        
                                DELETE #TempWebHook
                                FROM #TempWebHook TempWebHook INNER JOIN #TempWebHookDel TempWebHookDel ON TempWebHook.ID = TempWebHookDel.Id
                            END
                            DROP TABLE #TempWebHook
                            DROP TABLE #TempWebHookDel
                            GO
PRINT 'Running accountId 1 and table [Venda_Tabela_Produto_Parte]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempVenda_Tabela_Produto_Parte

                            CREATE TABLE #TempVenda_Tabela_Produto_Parte
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempVenda_Tabela_Produto_Parte (Id)
                            SELECT  [VTPP].Id
                            FROM [Venda_Tabela_Produto_Parte] VTPP INNER JOIN [Venda_Tabela_Produto] VTP ON VTPP.[ID_VendaTabelaProduto] = VTP.[ID] INNER JOIN [Venda] V ON VTP.[ID_Venda] = V.[ID]
                            WHERE [V].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempVenda_Tabela_Produto_ParteDel
                            CREATE TABLE #TempVenda_Tabela_Produto_ParteDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempVenda_Tabela_Produto_Parte))
                            BEGIN
                                TRUNCATE TABLE #TempVenda_Tabela_Produto_ParteDel

                                INSERT #TempVenda_Tabela_Produto_ParteDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempVenda_Tabela_Produto_Parte
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempVenda_Tabela_Produto_ParteDel
                                DELETE VTPP
                                FROM [Venda_Tabela_Produto_Parte] VTPP INNER JOIN #TempVenda_Tabela_Produto_ParteDel TempVenda_Tabela_Produto_ParteDel ON [VTPP].ID = TempVenda_Tabela_Produto_ParteDel.Id
                                WHERE [VTPP].ID >= @MinId AND [VTPP].ID <= @MaxId
                                        
                                DELETE #TempVenda_Tabela_Produto_Parte
                                FROM #TempVenda_Tabela_Produto_Parte TempVenda_Tabela_Produto_Parte INNER JOIN #TempVenda_Tabela_Produto_ParteDel TempVenda_Tabela_Produto_ParteDel ON TempVenda_Tabela_Produto_Parte.ID = TempVenda_Tabela_Produto_ParteDel.Id
                            END
                            DROP TABLE #TempVenda_Tabela_Produto_Parte
                            DROP TABLE #TempVenda_Tabela_Produto_ParteDel
                            GO
PRINT 'Running accountId 1 and table [Venda_Tabela_Produto]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempVenda_Tabela_Produto

                            CREATE TABLE #TempVenda_Tabela_Produto
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempVenda_Tabela_Produto (Id)
                            SELECT  [Venda_Tabela_Produto].Id
                            FROM [Venda_Tabela_Produto] Venda_Tabela_Produto
                            WHERE [Venda_Tabela_Produto].[AccountId] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempVenda_Tabela_ProdutoDel
                            CREATE TABLE #TempVenda_Tabela_ProdutoDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempVenda_Tabela_Produto))
                            BEGIN
                                TRUNCATE TABLE #TempVenda_Tabela_ProdutoDel

                                INSERT #TempVenda_Tabela_ProdutoDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempVenda_Tabela_Produto
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempVenda_Tabela_ProdutoDel
                                DELETE Venda_Tabela_Produto
                                FROM [Venda_Tabela_Produto] Venda_Tabela_Produto INNER JOIN #TempVenda_Tabela_ProdutoDel TempVenda_Tabela_ProdutoDel ON [Venda_Tabela_Produto].ID = TempVenda_Tabela_ProdutoDel.Id
                                WHERE [Venda_Tabela_Produto].ID >= @MinId AND [Venda_Tabela_Produto].ID <= @MaxId
                                        
                                DELETE #TempVenda_Tabela_Produto
                                FROM #TempVenda_Tabela_Produto TempVenda_Tabela_Produto INNER JOIN #TempVenda_Tabela_ProdutoDel TempVenda_Tabela_ProdutoDel ON TempVenda_Tabela_Produto.ID = TempVenda_Tabela_ProdutoDel.Id
                            END
                            DROP TABLE #TempVenda_Tabela_Produto
                            DROP TABLE #TempVenda_Tabela_ProdutoDel
                            GO
PRINT 'Running accountId 1 and table [Venda_Status]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempVenda_Status

                            CREATE TABLE #TempVenda_Status
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempVenda_Status (Id)
                            SELECT  [Venda_Status].Id
                            FROM [Venda_Status] Venda_Status
                            WHERE [Venda_Status].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempVenda_StatusDel
                            CREATE TABLE #TempVenda_StatusDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempVenda_Status))
                            BEGIN
                                TRUNCATE TABLE #TempVenda_StatusDel

                                INSERT #TempVenda_StatusDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempVenda_Status
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempVenda_StatusDel
                                DELETE Venda_Status
                                FROM [Venda_Status] Venda_Status INNER JOIN #TempVenda_StatusDel TempVenda_StatusDel ON [Venda_Status].ID = TempVenda_StatusDel.Id
                                WHERE [Venda_Status].ID >= @MinId AND [Venda_Status].ID <= @MaxId
                                        
                                DELETE #TempVenda_Status
                                FROM #TempVenda_Status TempVenda_Status INNER JOIN #TempVenda_StatusDel TempVenda_StatusDel ON TempVenda_Status.ID = TempVenda_StatusDel.Id
                            END
                            DROP TABLE #TempVenda_Status
                            DROP TABLE #TempVenda_StatusDel
                            GO
...
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempDocument_Page
                            ...
                            SELECT  [DP].Id
                            FROM [Document_Page] DP INNER JOIN [Venda] V ON DP.[OrderId] = V.[Id]
                            WHERE [V].[ID_ClientePloomes] = @AccountId
...
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempCotacao_Revisao_Aceita_Historico
                            ...
                            SELECT  [CRAH].Id
                            FROM [Cotacao_Revisao_Aceita_Historico] CRAH INNER JOIN [Venda] V ON CRAH.[ID_Venda] = V.ID
                            WHERE [V].[ID_ClientePloomes] = @AccountId
                            ...
                            GO
PRINT 'Running accountId 1 and table [Venda_Tabela]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempVenda_Tabela

                            CREATE TABLE #TempVenda_Tabela
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempVenda_Tabela (Id)
                            SELECT  [VT].Id
                            FROM [Venda_Tabela] VT INNER JOIN [Venda] V ON VT.[ID_Venda] = V.[ID]
                            WHERE [V].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempVenda_TabelaDel
                            CREATE TABLE #TempVenda_TabelaDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempVenda_Tabela))
                            BEGIN
                                TRUNCATE TABLE #TempVenda_TabelaDel

                                INSERT #TempVenda_TabelaDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempVenda_Tabela
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempVenda_TabelaDel
                                DELETE VT
                                FROM [Venda_Tabela] VT INNER JOIN #TempVenda_TabelaDel TempVenda_TabelaDel ON [VT].ID = TempVenda_TabelaDel.Id
                                WHERE [VT].ID >= @MinId AND [VT].ID <= @MaxId
                                        
                                DELETE #TempVenda_Tabela
                                FROM #TempVenda_Tabela TempVenda_Tabela INNER JOIN #TempVenda_TabelaDel TempVenda_TabelaDel ON TempVenda_Tabela.ID = TempVenda_TabelaDel.Id
                            END
                            DROP TABLE #TempVenda_Tabela
                            DROP TABLE #TempVenda_TabelaDel
                            GO
PRINT 'Running accountId 1 and table [Venda_Query]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempVenda_Query

                            CREATE TABLE #TempVenda_Query
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempVenda_Query (Id)
                            SELECT  [VQ].Id
                            FROM [Venda_Query] VQ INNER JOIN [Venda] V ON VQ.[OrderId] = V.[ID]
                            WHERE [V].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempVenda_QueryDel
                            CREATE TABLE #TempVenda_QueryDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempVenda_Query))
                            BEGIN
                                TRUNCATE TABLE #TempVenda_QueryDel

                                INSERT #TempVenda_QueryDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempVenda_Query
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempVenda_QueryDel
                                DELETE VQ
                                FROM [Venda_Query] VQ INNER JOIN #TempVenda_QueryDel TempVenda_QueryDel ON [VQ].ID = TempVenda_QueryDel.Id
                                WHERE [VQ].ID >= @MinId AND [VQ].ID <= @MaxId
                                        
                                DELETE #TempVenda_Query
                                FROM #TempVenda_Query TempVenda_Query INNER JOIN #TempVenda_QueryDel TempVenda_QueryDel ON TempVenda_Query.ID = TempVenda_QueryDel.Id
                            END
                            DROP TABLE #TempVenda_Query
                            DROP TABLE #TempVenda_QueryDel
                            GO
PRINT 'Running accountId 1 and table [Venda_Status_Historico]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempVenda_Status_Historico

                            CREATE TABLE #TempVenda_Status_Historico
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempVenda_Status_Historico (Id)
                            SELECT  [VSH].Id
                            FROM [Venda_Status_Historico] VSH INNER JOIN [Venda] V ON VSH.[ID_Venda] = V.[ID]
                            WHERE [V].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempVenda_Status_HistoricoDel
                            CREATE TABLE #TempVenda_Status_HistoricoDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempVenda_Status_Historico))
                            BEGIN
                                TRUNCATE TABLE #TempVenda_Status_HistoricoDel

                                INSERT #TempVenda_Status_HistoricoDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempVenda_Status_Historico
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempVenda_Status_HistoricoDel
                                DELETE VSH
                                FROM [Venda_Status_Historico] VSH INNER JOIN #TempVenda_Status_HistoricoDel TempVenda_Status_HistoricoDel ON [VSH].ID = TempVenda_Status_HistoricoDel.Id
                                WHERE [VSH].ID >= @MinId AND [VSH].ID <= @MaxId
                                        
                                DELETE #TempVenda_Status_Historico
                                FROM #TempVenda_Status_Historico TempVenda_Status_Historico INNER JOIN #TempVenda_Status_HistoricoDel TempVenda_Status_HistoricoDel ON TempVenda_Status_Historico.ID = TempVenda_Status_HistoricoDel.Id
                            END
                            DROP TABLE #TempVenda_Status_Historico
                            DROP TABLE #TempVenda_Status_HistoricoDel
                            GO
PRINT 'Running accountId 1 and table [Venda_Parcela]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempVenda_Parcela

                            CREATE TABLE #TempVenda_Parcela
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempVenda_Parcela (Id)
                            SELECT  [VP].Id
                            FROM [Venda_Parcela] VP INNER JOIN [Venda] V ON VP.[ID_Venda] = V.[ID]
                            WHERE [V].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempVenda_ParcelaDel
                            CREATE TABLE #TempVenda_ParcelaDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempVenda_Parcela))
                            BEGIN
                                TRUNCATE TABLE #TempVenda_ParcelaDel

                                INSERT #TempVenda_ParcelaDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempVenda_Parcela
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempVenda_ParcelaDel
                                DELETE VP
                                FROM [Venda_Parcela] VP INNER JOIN #TempVenda_ParcelaDel TempVenda_ParcelaDel ON [VP].ID = TempVenda_ParcelaDel.Id
                                WHERE [VP].ID >= @MinId AND [VP].ID <= @MaxId
                                        
                                DELETE #TempVenda_Parcela
                                FROM #TempVenda_Parcela TempVenda_Parcela INNER JOIN #TempVenda_ParcelaDel TempVenda_ParcelaDel ON TempVenda_Parcela.ID = TempVenda_ParcelaDel.Id
                            END
                            DROP TABLE #TempVenda_Parcela
                            DROP TABLE #TempVenda_ParcelaDel
                            GO
PRINT 'Running accountId 1 and table [Venda_Approval]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempVenda_Approval

                            CREATE TABLE #TempVenda_Approval
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempVenda_Approval (Id)
                            SELECT  [VA].Id
                            FROM [Venda_Approval] VA INNER JOIN [Venda] V ON VA.[OrderId] = V.[ID]
                            WHERE [V].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempVenda_ApprovalDel
                            CREATE TABLE #TempVenda_ApprovalDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempVenda_Approval))
                            BEGIN
                                TRUNCATE TABLE #TempVenda_ApprovalDel

                                INSERT #TempVenda_ApprovalDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempVenda_Approval
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempVenda_ApprovalDel
                                DELETE VA
                                FROM [Venda_Approval] VA INNER JOIN #TempVenda_ApprovalDel TempVenda_ApprovalDel ON [VA].ID = TempVenda_ApprovalDel.Id
                                WHERE [VA].ID >= @MinId AND [VA].ID <= @MaxId
                                        
                                DELETE #TempVenda_Approval
                                FROM #TempVenda_Approval TempVenda_Approval INNER JOIN #TempVenda_ApprovalDel TempVenda_ApprovalDel ON TempVenda_Approval.ID = TempVenda_ApprovalDel.Id
                            END
                            DROP TABLE #TempVenda_Approval
                            DROP TABLE #TempVenda_ApprovalDel
                            GO
PRINT 'Running accountId 1 and table [Venda]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempVenda

                            CREATE TABLE #TempVenda
                            (
                                Id INT PRIMARY KEY
                            )
                            ...
                            SELECT  [Venda].Id
                            FROM [Venda] Venda
                            WHERE [Venda].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempVendaDel
                            CREATE TABLE #TempVendaDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempVenda))
                            BEGIN
                                TRUNCATE TABLE #TempVendaDel

                                INSERT #TempVendaDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempVenda
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempVendaDel
                                DELETE Venda
                                FROM [Venda] Venda INNER JOIN #TempVendaDel TempVendaDel ON [Venda].ID = TempVendaDel.Id
                                WHERE [Venda].ID >= @MinId AND [Venda].ID <= @MaxId
                                        
                                DELETE #TempVenda
                                FROM #TempVenda TempVenda INNER JOIN #TempVendaDel TempVendaDel ON TempVenda.ID = TempVendaDel.Id
                            END
                            DROP TABLE #TempVenda
                            DROP TABLE #TempVendaDel
                            GO
PRINT 'Running accountId 1 and table [Usuario_Reset_Password_Control]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempUsuario_Reset_Password_Control

                            CREATE TABLE #TempUsuario_Reset_Password_Control
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempUsuario_Reset_Password_Control (Id)
                            SELECT  [Usuario_Reset_Password_Control].Id
                            FROM [Usuario_Reset_Password_Control] Usuario_Reset_Password_Control
                            WHERE [Usuario_Reset_Password_Control].[AccountId] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempUsuario_Reset_Password_ControlDel
                            CREATE TABLE #TempUsuario_Reset_Password_ControlDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempUsuario_Reset_Password_Control))
                            BEGIN
                                TRUNCATE TABLE #TempUsuario_Reset_Password_ControlDel

                                INSERT #TempUsuario_Reset_Password_ControlDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempUsuario_Reset_Password_Control
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempUsuario_Reset_Password_ControlDel
                                DELETE Usuario_Reset_Password_Control
                                FROM [Usuario_Reset_Password_Control] Usuario_Reset_Password_Control INNER JOIN #TempUsuario_Reset_Password_ControlDel TempUsuario_Reset_Password_ControlDel ON [Usuario_Reset_Password_Control].ID = TempUsuario_Reset_Password_ControlDel.Id
                                WHERE [Usuario_Reset_Password_Control].ID >= @MinId AND [Usuario_Reset_Password_Control].ID <= @MaxId
                                        
                                DELETE #TempUsuario_Reset_Password_Control
                                FROM #TempUsuario_Reset_Password_Control TempUsuario_Reset_Password_Control INNER JOIN #TempUsuario_Reset_Password_ControlDel TempUsuario_Reset_Password_ControlDel ON TempUsuario_Reset_Password_Control.ID = TempUsuario_Reset_Password_ControlDel.Id
                            END
                            DROP TABLE #TempUsuario_Reset_Password_Control
                            DROP TABLE #TempUsuario_Reset_Password_ControlDel
                            GO
PRINT 'Running accountId 1 and table [Usuario_Perfil]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempUsuario_Perfil

                            CREATE TABLE #TempUsuario_Perfil
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempUsuario_Perfil (Id)
                            SELECT  [Usuario_Perfil].Id
                            FROM [Usuario_Perfil] Usuario_Perfil
                            WHERE [Usuario_Perfil].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempUsuario_PerfilDel
                            CREATE TABLE #TempUsuario_PerfilDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempUsuario_Perfil))
                            BEGIN
                                TRUNCATE TABLE #TempUsuario_PerfilDel

                                INSERT #TempUsuario_PerfilDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempUsuario_Perfil
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempUsuario_PerfilDel
                                DELETE Usuario_Perfil
                                FROM [Usuario_Perfil] Usuario_Perfil INNER JOIN #TempUsuario_PerfilDel TempUsuario_PerfilDel ON [Usuario_Perfil].ID = TempUsuario_PerfilDel.Id
                                WHERE [Usuario_Perfil].ID >= @MinId AND [Usuario_Perfil].ID <= @MaxId
                                        
                                DELETE #TempUsuario_Perfil
                                FROM #TempUsuario_Perfil TempUsuario_Perfil INNER JOIN #TempUsuario_PerfilDel TempUsuario_PerfilDel ON TempUsuario_Perfil.ID = TempUsuario_PerfilDel.Id
                            END
                            DROP TABLE #TempUsuario_Perfil
                            DROP TABLE #TempUsuario_PerfilDel
                            GO
PRINT 'Running accountId 1 and table [Usuario_PromoterScore_Reason]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempUsuario_PromoterScore_Reason

                            CREATE TABLE #TempUsuario_PromoterScore_Reason
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempUsuario_PromoterScore_Reason (Id)
                            SELECT  [UPR].Id
                            FROM [Usuario_PromoterScore_Reason] UPR 
							INNER JOIN [Usuario_PromoterScore] UP ON UPR.[PromoterScoreId] = UP.[Id] 
							INNER JOIN [Usuario] U ON UP.[UserId] = U.[ID]
                            WHERE [U].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempUsuario_PromoterScore_ReasonDel
                            CREATE TABLE #TempUsuario_PromoterScore_ReasonDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempUsuario_PromoterScore_Reason))
                            BEGIN
                                TRUNCATE TABLE #TempUsuario_PromoterScore_ReasonDel

                                INSERT #TempUsuario_PromoterScore_ReasonDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempUsuario_PromoterScore_Reason
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempUsuario_PromoterScore_ReasonDel
                                DELETE UPR
                                FROM [Usuario_PromoterScore_Reason] UPR INNER JOIN #TempUsuario_PromoterScore_ReasonDel TempUsuario_PromoterScore_ReasonDel ON [UPR].ID = TempUsuario_PromoterScore_ReasonDel.Id
                                WHERE [UPR].ID >= @MinId AND [UPR].ID <= @MaxId
                                        
                                DELETE #TempUsuario_PromoterScore_Reason
                                FROM #TempUsuario_PromoterScore_Reason TempUsuario_PromoterScore_Reason INNER JOIN #TempUsuario_PromoterScore_ReasonDel TempUsuario_PromoterScore_ReasonDel ON TempUsuario_PromoterScore_Reason.ID = TempUsuario_PromoterScore_ReasonDel.Id
                            END
                            DROP TABLE #TempUsuario_PromoterScore_Reason
                            DROP TABLE #TempUsuario_PromoterScore_ReasonDel
                            GO
PRINT 'Running accountId 1 and table [Usuario_Filtro_Tarefa_Classe]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempUsuario_Filtro_Tarefa_Classe

                            CREATE TABLE #TempUsuario_Filtro_Tarefa_Classe
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempUsuario_Filtro_Tarefa_Classe (Id)
                            SELECT  [UFTC].Id
                            FROM [Usuario_Filtro_Tarefa_Classe] UFTC INNER JOIN [Usuario] U ON UFTC.[ID_Usuario] = U.[ID]
                            WHERE [U].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempUsuario_Filtro_Tarefa_ClasseDel
                            CREATE TABLE #TempUsuario_Filtro_Tarefa_ClasseDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempUsuario_Filtro_Tarefa_Classe))
                            BEGIN
                                TRUNCATE TABLE #TempUsuario_Filtro_Tarefa_ClasseDel

                                INSERT #TempUsuario_Filtro_Tarefa_ClasseDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempUsuario_Filtro_Tarefa_Classe
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempUsuario_Filtro_Tarefa_ClasseDel
                                DELETE UFTC
                                FROM [Usuario_Filtro_Tarefa_Classe] UFTC INNER JOIN #TempUsuario_Filtro_Tarefa_ClasseDel TempUsuario_Filtro_Tarefa_ClasseDel ON [UFTC].ID = TempUsuario_Filtro_Tarefa_ClasseDel.Id
                                WHERE [UFTC].ID >= @MinId AND [UFTC].ID <= @MaxId
                                        
                                DELETE #TempUsuario_Filtro_Tarefa_Classe
                                FROM #TempUsuario_Filtro_Tarefa_Classe TempUsuario_Filtro_Tarefa_Classe INNER JOIN #TempUsuario_Filtro_Tarefa_ClasseDel TempUsuario_Filtro_Tarefa_ClasseDel ON TempUsuario_Filtro_Tarefa_Classe.ID = TempUsuario_Filtro_Tarefa_ClasseDel.Id
                            END
                            DROP TABLE #TempUsuario_Filtro_Tarefa_Classe
                            DROP TABLE #TempUsuario_Filtro_Tarefa_ClasseDel
                            GO
PRINT 'Running accountId 1 and table [Usuario_Suspenso_Historico]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempUsuario_Suspenso_Historico

                            CREATE TABLE #TempUsuario_Suspenso_Historico
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempUsuario_Suspenso_Historico (Id)
                            SELECT  [USH].Id
                            FROM [Usuario_Suspenso_Historico] USH INNER JOIN [Usuario] U ON USH.[ID_Usuario] = U.[ID]
                            WHERE [U].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempUsuario_Suspenso_HistoricoDel
                            CREATE TABLE #TempUsuario_Suspenso_HistoricoDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempUsuario_Suspenso_Historico))
                            BEGIN
                                TRUNCATE TABLE #TempUsuario_Suspenso_HistoricoDel

                                INSERT #TempUsuario_Suspenso_HistoricoDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempUsuario_Suspenso_Historico
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempUsuario_Suspenso_HistoricoDel
                                DELETE USH
                                FROM [Usuario_Suspenso_Historico] USH INNER JOIN #TempUsuario_Suspenso_HistoricoDel TempUsuario_Suspenso_HistoricoDel ON [USH].ID = TempUsuario_Suspenso_HistoricoDel.Id
                                WHERE [USH].ID >= @MinId AND [USH].ID <= @MaxId
                                        
                                DELETE #TempUsuario_Suspenso_Historico
                                FROM #TempUsuario_Suspenso_Historico TempUsuario_Suspenso_Historico INNER JOIN #TempUsuario_Suspenso_HistoricoDel TempUsuario_Suspenso_HistoricoDel ON TempUsuario_Suspenso_Historico.ID = TempUsuario_Suspenso_HistoricoDel.Id
                            END
                            DROP TABLE #TempUsuario_Suspenso_Historico
                            DROP TABLE #TempUsuario_Suspenso_HistoricoDel
                            GO
PRINT 'Running accountId 1 and table [Usuario_Responsavel]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempUsuario_Responsavel

                            CREATE TABLE #TempUsuario_Responsavel
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempUsuario_Responsavel (Id)
                            SELECT  [UR].Id
                            FROM [Usuario_Responsavel] UR INNER JOIN [Usuario] U ON UR.[ID_Usuario] = U.[ID]
                            WHERE [U].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempUsuario_ResponsavelDel
                            CREATE TABLE #TempUsuario_ResponsavelDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempUsuario_Responsavel))
                            BEGIN
                                TRUNCATE TABLE #TempUsuario_ResponsavelDel

                                INSERT #TempUsuario_ResponsavelDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempUsuario_Responsavel
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempUsuario_ResponsavelDel
                                DELETE UR
                                FROM [Usuario_Responsavel] UR INNER JOIN #TempUsuario_ResponsavelDel TempUsuario_ResponsavelDel ON [UR].ID = TempUsuario_ResponsavelDel.Id
                                WHERE [UR].ID >= @MinId AND [UR].ID <= @MaxId
                                        
                                DELETE #TempUsuario_Responsavel
                                FROM #TempUsuario_Responsavel TempUsuario_Responsavel INNER JOIN #TempUsuario_ResponsavelDel TempUsuario_ResponsavelDel ON TempUsuario_Responsavel.ID = TempUsuario_ResponsavelDel.Id
                            END
                            DROP TABLE #TempUsuario_Responsavel
                            DROP TABLE #TempUsuario_ResponsavelDel
                            GO
PRINT 'Running accountId 1 and table [Usuario_PromoterScore]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempUsuario_PromoterScore

                            CREATE TABLE #TempUsuario_PromoterScore
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempUsuario_PromoterScore (Id)
                            SELECT  [UP].Id
                            FROM [Usuario_PromoterScore] UP INNER JOIN [Usuario] U ON UP.[UserId] = U.[ID]
                            WHERE [U].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempUsuario_PromoterScoreDel
                            CREATE TABLE #TempUsuario_PromoterScoreDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempUsuario_PromoterScore))
                            BEGIN
                                TRUNCATE TABLE #TempUsuario_PromoterScoreDel

                                INSERT #TempUsuario_PromoterScoreDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempUsuario_PromoterScore
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempUsuario_PromoterScoreDel
                                DELETE UP
                                FROM [Usuario_PromoterScore] UP INNER JOIN #TempUsuario_PromoterScoreDel TempUsuario_PromoterScoreDel ON [UP].ID = TempUsuario_PromoterScoreDel.Id
                                WHERE [UP].ID >= @MinId AND [UP].ID <= @MaxId
                                        
                                DELETE #TempUsuario_PromoterScore
                                FROM #TempUsuario_PromoterScore TempUsuario_PromoterScore INNER JOIN #TempUsuario_PromoterScoreDel TempUsuario_PromoterScoreDel ON TempUsuario_PromoterScore.ID = TempUsuario_PromoterScoreDel.Id
                            END
                            DROP TABLE #TempUsuario_PromoterScore
                            DROP TABLE #TempUsuario_PromoterScoreDel
                            GO
PRINT 'Running accountId 1 and table [Usuario_PipelineSettings]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempUsuario_PipelineSettings

                            CREATE TABLE #TempUsuario_PipelineSettings
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempUsuario_PipelineSettings (Id)
                            SELECT  [UP].Id
                            FROM [Usuario_PipelineSettings] UP INNER JOIN [Usuario] U ON UP.[UserId] = U.[ID]
                            WHERE [U].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempUsuario_PipelineSettingsDel
                            CREATE TABLE #TempUsuario_PipelineSettingsDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempUsuario_PipelineSettings))
                            BEGIN
                                TRUNCATE TABLE #TempUsuario_PipelineSettingsDel

                                INSERT #TempUsuario_PipelineSettingsDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempUsuario_PipelineSettings
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempUsuario_PipelineSettingsDel
                                DELETE UP
                                FROM [Usuario_PipelineSettings] UP INNER JOIN #TempUsuario_PipelineSettingsDel TempUsuario_PipelineSettingsDel ON [UP].ID = TempUsuario_PipelineSettingsDel.Id
                                WHERE [UP].ID >= @MinId AND [UP].ID <= @MaxId
                                        
                                DELETE #TempUsuario_PipelineSettings
                                FROM #TempUsuario_PipelineSettings TempUsuario_PipelineSettings INNER JOIN #TempUsuario_PipelineSettingsDel TempUsuario_PipelineSettingsDel ON TempUsuario_PipelineSettings.ID = TempUsuario_PipelineSettingsDel.Id
                            END
                            DROP TABLE #TempUsuario_PipelineSettings
                            DROP TABLE #TempUsuario_PipelineSettingsDel
                            GO
PRINT 'Running accountId 1 and table [Usuario_Perfil_AllowedChatUser]'
DECLARE @AccountId INT = 1
 DROP TABLE IF EXISTS #TempUsuario_Perfil_AllowedChatUser

                            CREATE TABLE #TempUsuario_Perfil_AllowedChatUser
                            (
                                Id INT PRIMARY KEY
                            )

                            INSERT #TempUsuario_Perfil_AllowedChatUser (Id)
                            SELECT  [UPA].Id
                            FROM [Usuario_Perfil_AllowedChatUser] UPA INNER JOIN [Usuario] U ON UPA.[UserId] = U.[ID]
                            WHERE [U].[ID_ClientePloomes] = @AccountId
                                    
                            DROP TABLE IF EXISTS #TempUsuario_Perfil_AllowedChatUserDel
                            CREATE TABLE #TempUsuario_Perfil_AllowedChatUserDel
                            (
                                Id INT PRIMARY KEY
                            )

                            DECLARE @MaxId INT, @MinId INT
                            WHILE (EXISTS(SELECT 1 FROM #TempUsuario_Perfil_AllowedChatUser))
                            BEGIN
                                TRUNCATE TABLE #TempUsuario_Perfil_AllowedChatUserDel

                                INSERT #TempUsuario_Perfil_AllowedChatUserDel (Id)
                                SELECT TOP 5000 ID
                                FROM #TempUsuario_Perfil_AllowedChatUser
                                ORDER BY Id ASC
                                        
                                SELECT @MinId = min(id), @MaxId =  max(id)  From #TempUsuario_Perfil_AllowedChatUserDel
                                DELETE UPA
                                FROM [Usuario_Perfil_AllowedChatUser] UPA INNER JOIN #TempUsuario_Perfil_AllowedChatUserDel TempUsuario_Perfil_AllowedChatUserDel ON [UPA].ID = TempUsuario_Perfil_AllowedChatUserDel.Id
                                WHERE [UPA].ID >= @MinId AND [UPA].ID <= @MaxId
                                        
                                DELETE #TempUsuario_Perfil_AllowedChatUser
                                FROM #TempUsuario_Perfil_AllowedChatUser TempUsuario_Perfil_AllowedChatUser INNER JOIN #TempUsuario_Perfil_AllowedChatUserDel TempUsuario_Perfil_AllowedChatUserDel ON TempUsuario_Perfil_AllowedChatUser.ID = TempUsuario_Perfil_AllowedChatUserDel.Id
                            END
                            DROP TABLE #TempUsuario_Perfil_AllowedChatUser
                            DROP TABLE #TempUsuario_Perfil_AllowedChatUserDel
                            GO
