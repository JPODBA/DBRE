use BA_DBA
go
CREATE or ALTER  PROC PR_DBA_EXPURGO_USUARIO_PROMOTERSCORE_REASON
	@Debug bit = 1, 
	@AccountId INT,
	@timeout  Time = '02:45',
	@Top INT
/************************************************************************
 ...
 Data de att:      
 Funcionalidade: Faz o Expurgo de dados da tabela USUARIO_PROMOTERSCORE_REASON do Ploomes_CRM. 
*************************************************************************/
AS
BEGIN
	SET NOCOUNT ON

	IF EXISTS (SELECT 1 FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE AcountID = @AccountId AND SPROC = 'PR_DBA_EXPURGO_USUARIO_PROMOTERSCORE_REASON' AND CountFim = 0)
	BEGIN
		PRINT 'PR_DBA_EXPURGO_USUARIO_PROMOTERSCORE_REASON já zerou esse accountID. Segue o próximo!!'
		RETURN
	END
    ...
    FROM Ploomes_CRM.dbo.[USUARIO_PROMOTERSCORE_REASON] T
    JOIN Ploomes_CRM.dbo.[Usuario_PromoterScore]       UP ON T.[PromoterScoreId] = UP.[Id] 
	JOIN Ploomes_CRM.dbo.[Usuario]					    U ON UP.[UserId] = U.[ID]
	...
	Select @Data_Inicio, null, 'PR_DBA_EXPURGO_USUARIO_PROMOTERSCORE_REASON', @CountLinhas_Inicio, null, @AccountId			
	
	SELECT @ID_LogManutencao = id FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE DataInicio = @Data_Inicio and SPROC = 'PR_DBA_EXPURGO_USUARIO_PROMOTERSCORE_REASON'
	--- lOG Delete Historico  ------------------------------------------------------------------------------
