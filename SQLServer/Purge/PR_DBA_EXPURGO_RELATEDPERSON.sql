...
BEGIN
	...
	IF EXISTS (SELECT 1 FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE AcountID = @AccountId AND SPROC = 'PR_DBA_EXPURGO_RELATEDPERSON' AND CountFim = 0)
	BEGIN
		PRINT 'PR_DBA_EXPURGO_RELATEDPERSON já zerou esse accountID. Segue o próximo!'
		RETURN
	END
