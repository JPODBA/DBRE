use BA_DBA
go
CREATE or ALTER  PROC PR_DBA_EXPURGO_INTEGRACAO_TOTALVOICE_USUARIO
	@Debug bit = 1, 
	@AccountId INT,
	@timeout  Time = '02:45',
	@Top INT
/************************************************************************
 ...
 Data de att:      
 Funcionalidade: Faz o Expurgo de dados da tabela INTEGRACAO_TOTALVOICE_USUARIO do Ploomes_CRM. 
*************************************************************************/
AS
BEGIN
	SET NOCOUNT ON

	IF EXISTS (SELECT 1 FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE AcountID = @AccountId AND SPROC = 'PR_DBA_EXPURGO_INTEGRACAO_TOTALVOICE_USUARIO' AND CountFim = 0)
	BEGIN
		PRINT 'PR_DBA_EXPURGO_INTEGRACAO_TOTALVOICE_USUARIO já zerou esse accountID. Segue o próximo!'
		RETURN
	END
    ...
    FROM Ploomes_CRM.dbo.[INTEGRACAO_TOTALVOICE_USUARIO] T
	JOIN Ploomes_CRM.dbo.[Integracao_TotalVoice]         IT ON T.[ID_Integracao] = IT.[ID]
	...
	Select @Data_Inicio, null, 'PR_DBA_EXPURGO_INTEGRACAO_TOTALVOICE_USUARIO', @CountLinhas_Inicio, null, @AccountId			
	
	SELECT @ID_LogManutencao = id FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE DataInicio = @Data_Inicio and SPROC = 'PR_DBA_EXPURGO_INTEGRACAO_TOTALVOICE_USUARIO'
	--- lOG Delete Historico  ------------------------------------------------------------------------------
