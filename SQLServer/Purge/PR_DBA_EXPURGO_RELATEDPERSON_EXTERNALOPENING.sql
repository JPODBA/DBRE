use BA_DBA
go
CREATE or ALTER  PROC PR_DBA_EXPURGO_RELATEDPERSON_EXTERNALOPENING
	@Debug bit = 1, 
	@AccountId INT,
	@timeout  Time = '02:45',
	@Top INT
/************************************************************************
 ...
 Data de att:      
 Funcionalidade: Faz o Expurgo de dados da tabela RELATEDPERSON_EXTERNALOPENING do Ploomes_CRM. 
*************************************************************************/
AS
BEGIN
	SET NOCOUNT ON
		
	IF EXISTS (SELECT 1 FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE AcountID = @AccountId AND SPROC = 'PR_DBA_EXPURGO_RELATEDPERSON_EXTERNALOPENING' AND CountFim = 0)
	BEGIN
		PRINT 'PR_DBA_EXPURGO_RELATEDPERSON_EXTERNALOPENING já zerou esse accountID. Segue o próximo!'
		RETURN
	END
    ...
    FROM Ploomes_CRM.dbo.[RELATEDPERSON_EXTERNALOPENING] RE 
	JOIN Ploomes_CRM.dbo.[RelatedPerson]                  R ON RE.RelatedPersonId = R.Id 
	LEFT JOIN (SELECT ID AS ItemId, ID_ClientePloomes, 7 as EntityId 
			   ...
			   FROM Ploomes_CRM.dbo.[Cliente]) T 
	ON (T.EntityId = 7 AND T.ItemId = R.QuoteId) 
	...
	Select @Data_Inicio, null, 'PR_DBA_EXPURGO_RELATEDPERSON_EXTERNALOPENING', @CountLinhas_Inicio, null, @AccountId			
	
	SELECT @ID_LogManutencao = id FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE DataInicio = @Data_Inicio and SPROC = 'PR_DBA_EXPURGO_RELATEDPERSON_EXTERNALOPENING'
	--- lOG Delete Historico  ------------------------------------------------------------------------------
	...
	BEGIN
			...
			-- Exclui pelo index. 
			DELETE T
			FROM Ploomes_CRM.dbo.[RELATEDPERSON_EXTERNALOPENING] T 
