use BA_DBA
go
CREATE or ALTER  PROC PR_DBA_EXPURGO_VENDA_STATUS_HISTORICO
	@Debug bit = 1, 
	@AccountId INT,
	@timeout  Time = '02:45',
	@Top INT
/************************************************************************
 ...
 Data de att:      
 Funcionalidade: Faz o Expurgo de dados da tabela VENDA_STATUS_HISTORICO do Ploomes_CRM. 
*************************************************************************/
AS
BEGIN
	SET NOCOUNT ON

	IF EXISTS (SELECT 1 FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE AcountID = @AccountId AND SPROC = 'PR_DBA_EXPURGO_VENDA_STATUS_HISTORICO' AND CountFim = 0)
	BEGIN
		PRINT 'PR_DBA_EXPURGO_VENDA_STATUS_HISTORICO já zerou esse accountID. Segue o próximo!!'
		RETURN
	END
    ...
    FROM Ploomes_CRM.dbo.[VENDA_STATUS_HISTORICO] T
	...
	Select @Data_Inicio, null, 'PR_DBA_EXPURGO_VENDA_STATUS_HISTORICO', @CountLinhas_Inicio, null, @AccountId			
	
	SELECT @ID_LogManutencao = id FROM BA_DBA.DBO.DBA_LOG_EXPURGO WHERE DataInicio = @Data_Inicio and SPROC = 'PR_DBA_EXPURGO_VENDA_STATUS_HISTORICO'
	--- lOG Delete Historico  ------------------------------------------------------------------------------
